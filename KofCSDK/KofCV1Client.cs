...
public class KofCV1Client : IKofCV1Client
{
    ...
    public KofCV1Client(HttpClient httpClient)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            using (var httpRequest = new HttpRequestMessage(HttpMethod.Post, $"api/{tenantInfo.TenantId}/v1.0/accounts/login"))
            {
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            using (var httpRequest = new HttpRequestMessage(HttpMethod.Get, $"api/{tenantInfo.TenantId}/v1.0/accounts/passwordRequirements"))
            {
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            using (var httpRequest = new HttpRequestMessage(HttpMethod.Get, $"api/{tenantInfo.TenantId}/v1.0/activities"))
            {
                SetAuthorizationHeader(httpRequest, userAuthentication.WebToken);
    ...
    public async Task<Result<Activity>> CreateActivity(
        TenantInfo tenantInfo,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            using (var content = SerializeRequest(request))
            using (var httpRequest = new HttpRequestMessage(HttpMethod.Post, $"api/{tenantInfo.TenantId}/v1.0/activities"))
            {
                SetAuthorizationHeader(httpRequest, request.UserAuthentication.WebToken);
                httpRequest.Content = content;

                using (var httpResponseMesage = await _httpClient.SendAsync(httpRequest, cancellationToken))
                {
                    return await ProcessResponse<Activity>(httpResponseMesage);
                }
            }
        }
        catch (Exception exception)
        {
            return HandleError<Activity>(exception);
        }
    }

    public async Task<Result<List<Knight>>> GetAllKnights(
        TenantInfo tenantInfo,
        UserAuthentication userAuthentication,
        CancellationToken cancellationToken = default)
    {
        try
        {
            using (var httpRequest = new HttpRequestMessage(HttpMethod.Get, $"api/{tenantInfo.TenantId}/v1.0/knights"))
            {
                SetAuthorizationHeader(httpRequest, userAuthentication.WebToken);

                using (var httpResponseMesage = await _httpClient.SendAsync(httpRequest, cancellationToken))
                {
                    return await ProcessResponse<List<Knight>>(httpResponseMesage);
                }
            }
        }
        catch (Exception exception)
        {
            return HandleError<List<Knight>>(exception);
        }
    }
